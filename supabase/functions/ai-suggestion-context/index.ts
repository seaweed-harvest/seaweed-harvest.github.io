...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const { data: run, error: runError } = await admin
      ...
      .select("id,feedback_id,attempt_number,mode,state,trust_tier,can_auto_plan,can_auto_implement,can_auto_merge,maximum_risk_level,source_app,target_key,target_repository,target_branch,started_at,created_at")
      .eq("id", runId)
      .single();
    if (runError || !run) throw runError ?? new PublicError(404, "Automation run not found");

    if (!["dispatched", "shadow_assessment_pending", "dispatch_pending", "assessing"].includes(run.state)) {
      throw new PublicError(409, `Automation run cannot be assessed from state ${run.state}`);
    }

    const { data: feedback, error: feedbackError } = await admin
      .from("ag_site_feedback")
      .select("id,created_at,source_app,source_page,page_url,feedback_type,message,locale,submitter_user_id,automation_enabled")
    ...
    await Promise.all([
      admin
        .from("ag_site_feedback_automation_runs")
        .update({ state: "assessing", started_at: run.started_at ?? now, updated_at: now })
        .eq("id", run.id),
      admin
        .from("ag_site_feedback")
        .update({ automation_status: "assessing", automation_last_processed_at: now })
        .eq("id", feedback.id)
    ]);
    ...
    return jsonResponse({
      ...
      automation_run: {
        ...
        permissions: {
          can_auto_plan: run.can_auto_plan,
          can_auto_implement: run.can_auto_implement,
          can_auto_merge: run.can_auto_merge,
          maximum_risk_level: run.maximum_risk_level
        }
      },
      suggestion: {
        id: feedback.id,
        ...
        authenticated_submitter: Boolean(feedback.submitter_user_id)
      },
      execution_authority: {
    ...
    console.error("ai-suggestion-context failed", error);
    const status = error instanceof PublicError ? error.status : 500;
    const message = error instanceof PublicError ? error.message : "Suggestion context could not be loaded";
