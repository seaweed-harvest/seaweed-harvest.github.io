...
create or replace function public.ag_reef_seaweed_workspace_submit_current(
  p_record_id uuid,
...
begin
  ...
  v_payload := public.ag_reef_seaweed_workspace_validate(p_record, p_units);
  ...
  if found then
    ...
    insert into public.ag_reef_seaweed_records (
      ...
      (v_record ->> 'record_date')::date,
      v_record ->> 'location', v_record ->> 'recorded_by_name',
      v_actor_id, 'submitted', clock_timestamp()
  ...
  if not v_created then
    ...
    set record_date = (v_record ->> 'record_date')::date,
  ...
  insert into public.ag_audit_log (
    ...
    case when v_created then 'reef_seaweed_record_created' else 'reef_seaweed_record_submitted' end,
...
create or replace function public.ag_reef_inspection_workspace_submit_current(
  p_record_id uuid,
  p_submission_id uuid,
  p_record jsonb,
  p_rafts jsonb
...
begin
  ...
  v_payload := public.ag_reef_inspection_workspace_validate(p_record, p_rafts);
  v_record := v_payload -> 'record';
  ...
  if found then
    ...
    insert into public.ag_reef_inspection_records (
      ...
      p_submission_id, v_aggregator_id,
      (v_record ->> 'inspection_date')::date,
      v_record ->> 'location', v_record ->> 'recorded_by_name',
      v_actor_id, nullif(v_record ->> 'general_notes', ''),
  ...
  if not v_created then
    ...
    set inspection_date = (v_record ->> 'inspection_date')::date,
        location = v_record ->> 'location',
        recorded_by_name = v_record ->> 'recorded_by_name',
  ...
  insert into public.ag_audit_log (
    ...
    v_actor_id, v_actor_email,
    case when v_created then 'reef_inspection_record_created' else 'reef_inspection_record_submitted' end,
