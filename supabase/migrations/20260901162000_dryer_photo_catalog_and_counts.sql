create or replace view public.seaweed_drying_photo_catalog
...
select
  ...
  submission.enumerator_name as recorder_name,
  submission.recorded_at,
  submission.record_status,
  submission.recorded_at as taken_at,
  (submission.recorded_at at time zone 'Africa/Nairobi')::date as activity_date,
  null::smallint as bay_number,
  'table'::text as phase,
  photo.storage_path,
  photo.photo_order::integer,
  0::integer as phase_sort
from public.seaweed_drying_submissions submission
cross join lateral unnest(submission.table_photo_paths)
  with ordinality photo(storage_path, photo_order)
...
select
  ...
  submission.recorded_at,
  submission.record_status,
  coalesce(bay.loading_at, submission.recorded_at) as taken_at,
  (coalesce(bay.loading_at, submission.recorded_at) at time zone 'Africa/Nairobi')::date as activity_date,
  bay.bay_number,
  'loading'::text as phase,
  photo.storage_path,
  photo.photo_order::integer,
  1::integer as phase_sort
from public.seaweed_drying_submissions submission
join public.seaweed_drying_bay_records bay
  on bay.submission_id = submission.id
cross join lateral unnest(bay.loading_photo_paths)
  with ordinality photo(storage_path, photo_order)
...
select
  ...
  submission.recorded_at,
  submission.record_status,
  coalesce(bay.unloading_at, submission.recorded_at) as taken_at,
  (coalesce(bay.unloading_at, submission.recorded_at) at time zone 'Africa/Nairobi')::date as activity_date,
  bay.bay_number,
  'unloading'::text as phase,
  photo.storage_path,
  photo.photo_order::integer,
  2::integer as phase_sort
from public.seaweed_drying_submissions submission
join public.seaweed_drying_bay_records bay
  on bay.submission_id = submission.id
cross join lateral unnest(bay.unloading_photo_paths)
  with ordinality photo(storage_path, photo_order);
...
comment on view public.seaweed_drying_photo_catalog is
  'Service-only catalog of photo paths already referenced by Dryer Table submissions and bay records. The underlying bucket remains private.';
...
begin
  ...
  from (
    select
      ...
      coalesce(cardinality(submission.table_photo_paths), 0)::integer as table_photo_count,
      coalesce(cardinality(bay.loading_photo_paths), 0)::integer as loading_photo_count,
      coalesce(cardinality(bay.unloading_photo_paths), 0)::integer as unloading_photo_count,
      (
        coalesce(cardinality(bay.loading_photo_paths), 0)
        + coalesce(cardinality(bay.unloading_photo_paths), 0)
      )::integer as photo_count
    from public.seaweed_drying_bay_summary summary
    join public.seaweed_drying_submissions submission
...
comment on function public.list_authenticated_seaweed_drying_ledger(text, integer) is
  'Read-only COSME protected-owner dryer ledger. Adds table/loading/unloading photo counts while retaining the existing cross-project authentication guard and performing no source-data mutations.';
