...
grant execute on function public.list_authenticated_seaweed_drying_ledger(text, integer) to anon, authenticated;

comment on function public.list_authenticated_seaweed_drying_ledger(text, integer) is
  'Read-only COSME protected-owner dryer ledger. Validates the Seaweed Harvest account-project access token before returning existing dryer records; performs no data mutations.';
