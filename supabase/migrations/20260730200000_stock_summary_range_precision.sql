...
as $$
  select case
    ...
    when p_max is null or p_min = p_max
      then to_char(p_min, 'FM999999999999990.999')
    else
      to_char(p_min, 'FM999999999999990.999')
      || ' - '
      || to_char(p_max, 'FM999999999999990.999')
  end
  || case
...
comment on function public.ag_operational_range_label(numeric, numeric, text) is
  'Formats a single operational value or min-max range while preserving up to three decimal places.';
