...
function initialiseFeedbackWidget() {
    ...
    <dialog class="site-feedback-dialog" aria-labelledby="siteFeedbackTitle">
      <form class="site-feedback-form" method="dialog" novalidate>
        ...
        <div class="site-feedback-photo-preview" hidden>
          <img alt="Selected suggestion photo">
  ...
  widget = { root, dialog, form, launcher, copy, photo: null, photoUrl: null };
  flushFeedbackQueue();
}
...
async function prepareFeedbackPhoto(event) {
  ...
  try {
    ...
    widget.photo = { blob, dataUrl, name: file.name || "suggestion-photo.jpg" };
...
async function compressFeedbackPhoto(file) {
  ...
  throw new Error("The photo is still larger than 700 KB. Try a smaller image.");
}
...
function injectStyles() {
  ...
  styles.textContent = `
    ...
    .site-feedback-photo-field small{color:#718784;font-size:.78rem;font-weight:400;line-height:1.35;text-transform:none}
    .site-feedback-photo-preview{display:grid;grid-template-columns:72px 1fr;align-items:center;gap:10px;padding:9px;border:1px solid var(--sf-line);border-radius:6px;background:var(--sf-soft)}
    .site-feedback-photo-preview[hidden]{display:none}
    .site-feedback-photo-preview img{width:72px;height:58px;border-radius:4px;object-fit:cover;background:#e9f3f2}
    .site-feedback-photo-remove{justify-self:start;min-height:36px;padding:7px 11px;border:1px solid var(--sf-line);border-radius:6px;background:#fff;color:var(--sf-ink);font:inherit;font-weight:700;cursor:pointer}
