...
import {
  ...
  setupPhotoViewer,
  signedPhotoUrl
} from "./photo_viewer_v3.js?v=1";
import { fetchDryerPhotoLibrary } from "./dryer_photo_client.js?v=2";

const DEFAULT_PAGE_SIZE = 20;
const PAGE_SIZE_OPTIONS = new Set([20, 40, 60]);
const PAGE_SIZE_STORAGE_KEY = "seaweed_harvest_photo_page_size_v1";
...
const state = {
  ...
  total: 0,
  page: 0,
  pageSize: readPageSizePreference(),
  sort: "taken_at",
  direction: "desc",
...
async function init() {
  cacheElements();
  if (els.photoPageSize) els.photoPageSize.value = String(state.pageSize);
...
function bindEvents() {
  ...
  els.photoPageSize?.addEventListener("change", () => {
    const nextSize = normalisePageSize(els.photoPageSize.value);
    state.pageSize = nextSize;
    els.photoPageSize.value = String(nextSize);
    storePageSizePreference(nextSize);
    state.page = 0;
    void loadPhotos();
  });
  ...
  els.photoLibraryRows.addEventListener("click", (event) => {
    ...
    if (openButton) {
      void openPagePhoto(Number(openButton.dataset.photoOpen));
      return;
    }
    if (event.target.closest("button, a, input, select")) return;
    const row = event.target.closest("[data-photo-row]");
    if (!row) return;
    void openPagePhoto(Number(row.dataset.photoRow));
  });
...
function configureCosmeMode() {
  ...
  const locationHeadingButton = els.photoCommunityHeading?.querySelector("button");
  if (locationHeadingButton) {
    locationHeadingButton.textContent = "Location";
    locationHeadingButton.dataset.photoSort = "location";
  }
...
async function loadPhotos() {
  ...
  try {
    ...
    const lastPage = state.total > 0 ? Math.floor((state.total - 1) / state.pageSize) : 0;
...
async function loadGenericPhotos() {
  const { data, error } = await authClient.rpc("ag_photo_library", {
    ...
    p_page_limit: state.pageSize,
    p_page_offset: state.page * state.pageSize
  });
  if (error) throw error;
...
function render() {
  ...
  els.photoNext.disabled = (state.page + 1) * state.pageSize >= state.total;
  syncViewMode();
  if (state.galleryVisible) void renderGallery();
}
...
async function openPagePhoto(index) {
  if (!Number.isInteger(index) || index < 0 || index >= state.rows.length) return;
  const sequence = ++state.viewerLoadSequence;
  setStatus("Preparing photo viewer...");
  try {
    const items = await Promise.all(state.rows.map(async (row, rowIndex) => ({
      url: row.signed_url || await signedPhotoUrl(row.bucket_id, row.storage_path),
      title: photoTitle(row),
      caption: photoCaption(row),
      alt: `${photoTitle(row)} — photo ${state.page * state.pageSize + rowIndex + 1}`
    })));
    if (sequence !== state.viewerLoadSequence) return;
    setStatus("");
    openPhotoUrlPreview(items, sourceLabelForRows(), index);
  } catch (error) {
    if (sequence !== state.viewerLoadSequence) return;
    setStatus(error?.message || "Photo preview could not be loaded.", "error");
  }
}
...
async function renderGallery() {
  ...
  await Promise.all(state.rows.map(async (row, index) => {
    try {
      const url = row.signed_url || await signedPhotoUrl(row.bucket_id, row.storage_path);
      if (state.rows !== expectedRows) return;
      const button = document.createElement("button");
      button.type = "button";
      button.className = "photo-library-thumbnail";
      button.title = photoTitle(row);
      ...
      if (state.cosmeMode) {
        ...
        context.textContent = [row.location, row.photo_context].filter(Boolean).join(" · ");
        button.append(context);
      }

      button.addEventListener("click", () => { void openPagePhoto(index); });
...
function photoTitle(row) {
  return [
    ...
    row.location || row.community_name,
    row.photo_context
  ].filter(Boolean).join(" — ") || "Photo";
}

function photoCaption(row) {
  const details = [formatDateTime(row.taken_at)];
  if (!state.cosmeMode) {
    details.push(row.community_name, row.recorder_name, row.seaweed_type, row.grade_code ? `Grade ${row.grade_code}` : "");
  } else {
    details.push(row.location, row.photo_context, row.recorder_name);
  }
  return details.filter(Boolean).join(" · ");
}

function sourceLabelForRows() {
  if (state.cosmeMode) return sourceLabel();
  const labels = [...new Set(state.rows.map((row) => row.source_label).filter(Boolean))];
  return labels.length === 1 ? labels[0] : "Photos";
}
...
function readPageSizePreference() {
  try {
    return normalisePageSize(window.localStorage.getItem(PAGE_SIZE_STORAGE_KEY));
  } catch {
    return DEFAULT_PAGE_SIZE;
  }
}

function storePageSizePreference(pageSize) {
  try {
    window.localStorage.setItem(PAGE_SIZE_STORAGE_KEY, String(pageSize));
  } catch {
    // Browser storage can be unavailable in private/restricted contexts; keep the in-memory choice.
  }
}
