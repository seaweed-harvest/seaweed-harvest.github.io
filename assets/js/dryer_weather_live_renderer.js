...
function interceptControls(event) {
  ...
  if (rangeButton) {
    ...
    document.querySelectorAll("[data-analysis-range]").forEach((button) => button.classList.toggle("active", button === rangeButton));
    render();
    return;
  }
  if (tideButton) {
...
async function waitForPlotly() {
  ...
  while (!window.Plotly && Date.now() - startedAt < 12000) await new Promise((resolve) => window.setTimeout(resolve, 100));
...
function buildRuns(rows) {
  ...
  }).filter((run) => run.start && Number.isFinite(Date.parse(run.start))).sort((a, b) => Date.parse(a.start) - Date.parse(b.start));
}
...
function bounds() {
  ...
  const liveEnd = state.runs.reduce((latestValue, run) => Math.max(latestValue, Date.parse(run.end || run.start) || 0), 0);
  const end = new Date(Math.max(snapshotEnd, liveEnd));
  return [new Date(end.getTime() - state.days * 86400000), end];
}
...
function weatherTraces() {
  ...
  const traces = [
    { x: selected.w.x, y: selected.w.temp, type: "scatter", mode: "lines", name: "Temperature", line: { color: "#e78a55", width: 2.7, shape: "spline", smoothing: 0.28 }, yaxis: "y", hovertemplate: `${fmt}<br><b>%{y:.1f} °C</b><extra>Temperature</extra>` },
    { x: selected.w.x, y: selected.w.hum, type: "scatter", mode: "lines", name: "Humidity", line: { color: "#4ca58f", width: 2.5, shape: "spline", smoothing: 0.28 }, yaxis: "y2", hovertemplate: `${fmt}<br><b>%{y:.0f}% RH</b><extra>Humidity</extra>` },
    { x: D.rain.x, y: D.rain.y, customdata: D.rain.source, type: "bar", name: "Rainfall", marker: { color: "#7da7d4" }, opacity: 0.5, yaxis: "y3", hovertemplate: "%{x|%a %d %b}<br><b>%{y:.1f} mm</b><br>%{customdata}<extra>Regional rainfall</extra>" }
  ];
  if (state.tide) traces.push({ x: TIDE.x, y: TIDE.height, customdata: TIDE.date.map((date, index) => [date, TIDE.time[index]]), type: "scatter", mode: "lines", name: "Lowest low tide", line: { color: "#687ea6", width: 2.2, shape: "spline", smoothing: 0.45 }, yaxis: "y4", hovertemplate: "%{customdata[0]}<br>Low tide: <b>%{y:.2f} m</b> at %{customdata[1]} EAT<extra>Daily lowest low</extra>" });
  return traces;
}

function weatherLayout(start, end) {
  return {
    paper_bgcolor: "#fff", plot_bgcolor: "#fff", margin: { l: 105, r: 135, t: 45, b: 12 }, hovermode: "x", dragmode: false, bargap: 0.18,
    legend: { orientation: "h", x: 0, y: 1.04, xanchor: "left", yanchor: "bottom" },
    xaxis: { type: "date", range: [start, end], showticklabels: false, showgrid: true, gridcolor: "rgba(70,90,85,.08)", zeroline: false, fixedrange: true },
    yaxis: { title: { text: "Temperature" }, side: "left", fixedrange: true, showgrid: true, gridcolor: "rgba(70,90,85,.10)", zeroline: false, ticksuffix: " °C", tickformat: ".0f", dtick: 1 },
    yaxis2: { overlaying: "y", side: "right", title: { text: "Humidity" }, fixedrange: true, range: [45, 100], showgrid: false, zeroline: false, ticksuffix: " %", tickformat: ".0f", dtick: 10 },
    yaxis3: { overlaying: "y", anchor: "free", position: 0.96, side: "right", title: { text: "Rainfall" }, fixedrange: true, rangemode: "tozero", showgrid: false, zeroline: false, ticksuffix: " mm", tickformat: ".0f" },
    yaxis4: { overlaying: "y", anchor: "free", position: 0.02, side: "left", title: { text: "Low tide", font: { color: "#687ea6" } }, fixedrange: true, range: [0, 1.6], visible: state.tide, showgrid: false, zeroline: false, ticksuffix: " m", tickformat: ".1f", tickfont: { color: "#687ea6" } },
    shapes: harvestShapes(start, end), annotations: [{ xref: "paper", yref: "paper", x: 0, y: 1.06, text: "<b>Weather</b>", showarrow: false, xanchor: "left", font: { size: 13, color: "#526a63" } }]
  };
}
...
function harvestShapes(start, end) {
  ...
  TIDE.date.forEach((date, index) => {
    ...
    shapes.push({ type: "rect", xref: "x", yref: "paper", x0: from.toISOString(), x1: to.toISOString(), y0: 0, y1: 1, fillcolor: "rgba(86,170,102,.11)", line: { width: 0 }, layer: "below" });
  });
  return shapes;
}

function runTraces(rangeEnd) {
  return state.runs.map((run) => ({ x: [run.start, run.end || rangeEnd.toISOString()], y: [run.table, run.table], type: "scatter", mode: "lines", line: { color: COLORS[run.table] || "#7c8f89", width: 13 }, showlegend: false, hovertemplate: `<b>${escapeHtml(run.table)}</b><br>${formatDate(run.start)} → ${run.end ? formatDate(run.end) : "in progress"}<br>Underlying drying records: ${run.records}<extra></extra>` }));
}
...
function runLayout(start, end) {
  ...
  return {
    paper_bgcolor: "#fff", plot_bgcolor: "#fff", margin: { l: 105, r: 135, t: 45, b: 16 }, hovermode: "closest", dragmode: false,
    xaxis: { type: "date", range: [start, end], side: "top", fixedrange: true, showgrid: true, gridcolor: "rgba(70,90,85,.09)", zeroline: false, tickformat: "%a<br><b>%d %b</b>", dtick: state.days === 7 ? 86400000 : state.days === 30 ? 3 * 86400000 : 7 * 86400000 },
    yaxis: { type: "category", fixedrange: true, categoryorder: "array", categoryarray: [...tables].reverse(), showgrid: false, zeroline: false, tickfont: { size: 11 } }
  };
}

function plotConfig() { return { responsive: true, displaylogo: false, scrollZoom: false, doubleClick: false, displayModeBar: false }; }
function syncTideUi() { const tideButton = document.getElementById("dryerAnalysisTideToggle"); const tidePill = document.getElementById("dryerAnalysisTidePill"); if (tideButton) tideButton.classList.toggle("active", state.tide); if (tidePill) tidePill.hidden = !state.tide; }
function ensureRendered() { const chart = document.getElementById("dryerAnalysisWeatherChart"); if (!chart || chart.querySelector(".dryer-weather-v2-root")) return; if (window.Plotly && state.runs.length) render(); }
function showRenderError(error) { const chart = document.getElementById("dryerAnalysisWeatherChart"); if (!chart) return; chart.innerHTML = `<div style="padding:18px;color:#8a3f3f;font-size:.8rem">Timeline + Weather could not render. ${escapeHtml(error?.message || "Unknown chart error")}</div>`; }
function waitForElement(id, timeoutMs) { const existing = document.getElementById(id); if (existing) return Promise.resolve(existing); return new Promise((resolve, reject) => { const observer = new MutationObserver(() => { const element = document.getElementById(id); if (!element) return; observer.disconnect(); resolve(element); }); observer.observe(document.body, { childList: true, subtree: true }); window.setTimeout(() => { observer.disconnect(); reject(new Error(`${id} was not created.`)); }, timeoutMs); }); }
...
function formatDate(value) { const date = new Date(value); if (Number.isNaN(date.getTime())) return "—"; return date.toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric", timeZone: KENYA_TIME_ZONE }); }
